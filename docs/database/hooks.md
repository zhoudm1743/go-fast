...
| 接口 | 方法 | 触发时机 |
|------|------|---------|
| `contracts.BeforeCreator` | `BeforeCreate(q Query) error` | 创建前 |
| `contracts.AfterCreator` | `AfterCreate(q Query) error` | 创建后 |
| `contracts.BeforeUpdater` | `BeforeUpdate(q Query) error` | 更新前 |
| `contracts.AfterUpdater` | `AfterUpdate(q Query) error` | 更新后 |
| `contracts.BeforeDeleter` | `BeforeDelete(q Query) error` | 删除前 |
| `contracts.AfterDeleter` | `AfterDelete(q Query) error` | 删除后 |
| `contracts.AfterFinder` | `AfterFind(q Query) error` | 查询后 |
