...
| ID 自动生成 | 框架自动调用 `AutoGenerateID()`，自定义 `OnBeforeCreate` 无需手动处理 ID |
| 钩子不影响 Raw/Exec | 原生 SQL 不触发任何模型钩子 |


---

## 内置钩子：BeforeCreate 自动生成时序 ID

`database.Model` 已内置 `BeforeCreate`，会在创建时自动生成时序 ID 主键（16 字符）：
...
| 调用父类钩子 | 自定义 `BeforeCreate` 时应先调用 `u.Model.BeforeCreate(q)` 确保 ID 生成 |
| 钩子不影响 Raw/Exec | 原生 SQL 不触发任何模型钩子 |
